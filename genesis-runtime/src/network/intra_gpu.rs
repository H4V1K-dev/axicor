...
impl IntraGpuChannel {
    ...
    /// В production заменяется `sync_ghosts` (launch_ghost_sync).
    pub fn sync_spikes(&mut self, zones: &mut [crate::zone_runtime::ZoneRuntime]) {
        for i in 0..self.count as usize {
            let src_zone = self.src_zone_indices[i];
            let dst_zone = self.dst_zone_indices[i];
            let src_axon = self.src_indices_host[i];
            let dst_ghost = self.dst_indices_host[i];

            unsafe {
                let src_ptr = (zones[src_zone].runtime.vram.axon_head_index as *const u32)
                    .add(src_axon as usize);
                let dst_ptr = (zones[dst_zone].runtime.vram.axon_head_index as *mut u32)
                    .add(dst_ghost as usize);
                std::ptr::write(dst_ptr, std::ptr::read(src_ptr));
            }
        }
    }
