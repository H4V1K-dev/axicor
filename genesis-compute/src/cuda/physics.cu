...
__global__ void cu_reset_burst_counters_kernel(ShardVramPtrs vram,
                                               uint32_t padded_n) {
...
// ============================================================================
__global__ void
cu_inject_inputs_kernel(BurstHeads8 *__restrict__ axon_heads,
...
// ============================================================================
__global__ void
cu_apply_spike_batch_kernel(BurstHeads8 *__restrict__ axon_heads,
...
// Milestone 4: continuous or discrete adaptive leak, selected by
// adaptive_leak_mode
// ============================================================================
__global__ void cu_update_neurons_kernel(ShardVramPtrs vram, uint32_t padded_n,
  ...
  if (p.adaptive_leak_mode != 0 && p.leak_min < p.leak_max) {
    ...
    if (p.adaptive_leak_mode == 1) {
      ...
      int32_t base_clamped_lo =
          p.leak_rate - ((p.leak_rate - lo) & ((p.leak_rate - lo) >> 31));
      int32_t base_clamped = base_clamped_lo + ((hi - base_clamped_lo) &
                                                ((hi - base_clamped_lo) >> 31));
  ...
  int32_t leaked_abs = abs_diff - leak_used;
  ...
  // [DOD] Threshold Soft Cap: нейрон не может поднять порог выше 10x от
  // базового
  ...
  current_voltage = is_glif_spiking * p.rest_potential +
                    (1 - is_glif_spiking) * current_voltage;
  thresh_offset += is_glif_spiking * p.homeostasis_penalty;
  uint8_t new_timer = is_glif_spiking * p.refractory_period +
                      (1 - is_glif_spiking) * vram.timers[tid];

  // 7. Сдвиг голов аксона при спайке (Burst Shift)
  if (final_spike) {
    // [DOD FIX] Bit 0: Instant Spike (GSOP), Bit 1: Batch Accumulator
    // (Sprouting)
    ...
    // [DOD FIX] Очищаем ТОЛЬКО мгновенный спайк. Аккумулятор (Bit 1) остается
    // жив до Ночи.
    flags = flags & ~0x01;
  }
  ...
  uint8_t updated_burst_count = burst_count;
  updated_burst_count += final_spike * (updated_burst_count < 7);
  vram.soma_flags[tid] =
      (flags & 0xF0) | (updated_burst_count << 1) | (uint8_t)final_spike;
...
#pragma unroll
    ...
    // [DOD FIX] Branchless clamp(0, val). Если new_abs < 0, сдвиг даст
    // 0xFFFFFFFF, инверсия даст 0. Это предотвращает инверсию знака веса (
    // Dale's Law Safety ).
...
// ============================================================================
__global__ void
cu_record_readout_kernel(const uint8_t *__restrict__ soma_flags,
...
__global__ void cu_extract_telemetry_kernel(
  ...
  if (is_spiking) {
    // Вычисляем локальный индекс потока среди стреляющих (считаем единицы до
    // текущего бита)
...
// ============================================================================
int32_t
cu_step_day_phase(const ShardVramPtrs *vram, uint32_t padded_n,
                  uint32_t total_axons, uint32_t v_seg, uint32_t current_tick,
                  // --- ВХОДЫ (InjectInputs) ---
                  const uint32_t *input_bitmask, uint32_t virtual_offset,
                  uint32_t num_virtual_axons,
                  // --- СЕТЬ (ApplySpikeBatch) ---
                  const uint32_t *incoming_spikes, uint32_t num_incoming_spikes,
                  // --- ВЫХОДЫ (RecordReadout) ---
                  const uint32_t *mapped_soma_ids, uint8_t *output_history,
                  uint32_t num_outputs, int16_t dopamine, cudaStream_t stream) {
  int threads = 256;

  // Burst reset выполняется в shard.rs один раз в начале батча, не здесь.
  // Иначе burst_count сбрасывался бы каждый тик и не накапливался.
  ...
  cu_propagate_axons_kernel<<<blocks_axons, threads, 0, stream>>>(
      vram->axon_heads, total_axons, v_seg);

  // 4. UpdateNeurons (GLIF)
  int blocks_neurons = (padded_n + threads - 1) / threads;
  cu_update_neurons_kernel<<<blocks_neurons, threads, 0, stream>>>(
      *vram, padded_n, current_tick, dopamine);

  // 5. ApplyGSOP (Пластичность 3D STDP)
  cu_apply_gsop_kernel<<<blocks_neurons, threads, 0, stream>>>(*vram, padded_n,
                                                               dopamine);
...
extern "C" void cu_reset_burst_counters(const ShardVramPtrs *vram,
                                        uint32_t padded_n,
                                        cudaStream_t stream) {
  int threads = 256;
  int blocks = (padded_n + threads - 1) / threads;
  cu_reset_burst_counters_kernel<<<blocks, threads, 0, stream>>>(*vram,
                                                                 padded_n);
}
