...
// =============================================================================
// § VramShard — Владелец VRAM для одного шарда
//
// Правила:
...
pub struct VramShard {
    pub ptrs:        ShardVramPtrs,
    pub padded_n:    u32,
    pub total_axons: u32,
}

unsafe impl Send for VramShard {}
unsafe impl Sync for VramShard {}
    ...
    pub fn upload_axon_heads(&self, axon_heads_blob: &[u8]) {
        ...
        let ok = unsafe {
            ffi::gpu_memcpy_host_to_device(
                self.ptrs.axon_heads as *mut c_void,
                axon_heads_blob.as_ptr() as *const c_void,
                expected,
            )
        };
        assert!(ok, "FATAL: gpu_memcpy_host_to_device failed for axon_heads");
    }
}

impl Drop for VramShard {
    fn drop(&mut self) {
        unsafe { ffi::cu_free_shard(&mut self.ptrs) };
    }
}

// =============================================================================
// § Backward-Compatible Legacy API (используется genesis-node/src/boot.rs)
// =============================================================================
...
pub struct GxiData<'a> {
    pub axon_ids: &'a [u32],
    pub matrices: &'a [InputMatrixInfo],
}

pub struct GxoData<'a> {
    pub soma_ids: &'a [u32],
}
...
impl VramState {
    pub fn load_shard(state_bytes: &[u8], axons_bytes: &[u8], gxi: Option<GxiData>, gxo: Option<GxoData>, readout_batch_ticks: u32, input_stride: u32, required_ghost_slots: usize) -> anyhow::Result<Self> {
        ...
        if let Some(g) = gxi {
            ...
            if num_pixels > 0 {
                ...
                for matrix_desc in g.matrices {
                    let num_pix = matrix_desc.num_pixels;
                    input_matrices.push(matrix_desc.clone());
