...
use genesis_compute::memory::{VramState, GxiData, GxoData, InputMatrixInfo};
use genesis_compute::ShardEngine;
use genesis_compute::ffi as gpu_ffi;
...
impl Bootloader {
    ...
    pub async fn boot_node(manifest_path: &Path, dashboard: Arc<crate::tui::DashboardState>) -> Result<BootResult> {
        ...
        let words_per_tick = (island.num_inputs + 31) / 32;
        let sync_batch_ticks = 100;
        let input_bytes = (words_per_tick as usize) * sync_batch_ticks * 4;
        
        let pinned_input_ptr = unsafe { gpu_ffi::gpu_host_alloc(input_bytes) as *mut u32 };
    ...
    fn load_zone(manifest_path: &Path, dashboard: Arc<crate::tui::DashboardState>) -> Result<(ZoneRuntime, ShardEngine)> {
        let manifest_toml = std::fs::read_to_string(manifest_path)?;
        let manifest: ZoneManifest = toml::from_str(&manifest_toml)?;
        let baked_dir = manifest_path.parent().unwrap_or(std::path::Path::new("."));
        ...
        // Convert GxiFile → GxiData for load_shard
        let gxi_matrices_owned: Option<Vec<InputMatrixInfo>> = gxi.as_ref().map(|g| {
            g.matrices.iter().map(|m| InputMatrixInfo {
                pixel_offset: m.offset,
                num_pixels: (m.width as u32) * (m.height as u32),
                stride: m.stride,
            }).collect()
        });
        let gxi_data = gxi.as_ref().zip(gxi_matrices_owned.as_ref()).map(|(g, mats)| GxiData {
            axon_ids: &g.axon_ids,
            matrices: mats,
        });
        let gxo_data = gxo.as_ref().map(|g| GxoData { soma_ids: &g.soma_ids });
        ...
        let island = ShardEngine::new(
            vram.to_layout(),
            std::ptr::null(),
            vram.mapped_soma_ids as *const u32,
            vram.num_pixels,
