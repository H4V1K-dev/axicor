...
| `[3:1]` | `burst_count` | `u3` | **[BDP]** Счетчик серийных спайков (0..7). Инкрементируется при каждом спайке в рамках одного батча. Сбрасывается микро-ядром перед началом нового батча. | ✅ Активно |
| ``      | `is_spiking`  | `u1` | Бит срабатывания. 1 = сома генерирует спайк в текущем тике.                                                                                               | ✅ Активно |

**Инварианты BDP (Burst-Dependent Plasticity):**
Счетчик `burst_count` читается ядром пластичности для нелинейного умножения STDP. Сброс счетчика выполняется строго маской `0xF1` (`11110001_2`), чтобы выжечь биты `[3:1]`, не затронув `type_mask` и флаг текущего спайка.
