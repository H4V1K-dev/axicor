...
- `ghost_capacity` (`u32`) - Резерв VRAM под межшардовые связи (Ghost Axons).
  - **Генерация:** Рассчитывается компилятором (Python SDK) автоматически на основе суммарной площади входящих матриц `SUM(width * height) * 2.0`.
  - **Зачем:** Обеспечивает запас емкости для структурной пластичности связей из других зон в Ночную Фазу. Хардкод `200000` более не используется.
