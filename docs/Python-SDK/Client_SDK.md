...
UDP MTU ограничен 65507 байтами. Если ваш `payload_size` превышает этот лимит (например, передача плотного видеокадра), SDK-клиент обязан нарезать матрицу на чанки. Заголовок `ExternalIoHeader` прикрепляется к каждому чанку. Оркестратор Axicor сам соберет их в VRAM по указанному `payload_size`.
...
В архитектуре Genesis значение `target_packed == 0` — это аппаратный триггер Early Exit для GPU и MCU (означает абсолютно пустой слот). Из-за этого реальный `axon_id` аппаратно смещен на `+1`.
...
# [31..24] Segment Offset | [23..0] Axon ID + 1
